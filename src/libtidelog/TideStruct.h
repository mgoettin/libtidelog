...
struct HEADER {
    ...
    HEADER(const char stag[4], uint64_t block_size)  : tag({stag[0], stag[1], stag[2], stag[3]}), 
...
struct CHUNK {
    ...
    CHUNK(const uint32_t id, const uint32_t count, const timeval& start,
	  const timeval& end, uint8_t compression) : id(id), 
						     count(count),
						     start(timeval2tstamp(start)), 
						     end(timeval2tstamp(end)), 
